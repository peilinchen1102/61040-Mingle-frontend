...
async function createGroup(name: string, members: string) {
  const processedMembers = JSON.parse("[" + members + "]");
  console.log(processedMembers);
  try {
    await fetchy(`/api/groups`, "POST", { body: { name: name, members: processedMembers } });
  } catch (e) {
    return;
  }
  emptyForm();
...
<template>
  <form id="texting" @submit.prevent="createGroup(name, members)" class="pure-form">
    <div class="column" style="display: flex; align-items: flex-start">
      ...
      <div>
        <label>Members:</label>
        <input class="textbox" style="width: 15em" type="text" v-model="members" placeholder=' "[user1]", "[user2]", ...' required />
